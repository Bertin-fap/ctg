...
def create_justificatif(ymd,year,intitulle,somme):
    ...
    if os.path.isdir(ctg_path_finance):
        ...
        image_list = [Image.open(ctg_path_finance / Path(x)) for x in os.listdir(ctg_path_finance) if x.endswith(".xlsx")]
        for idx,file in enumerate(docx_list):
            excel = client.Dispatch("Excel.Application")
            sheets = excel.Workbooks.Open(file)
            work_sheets = sheets.Worksheets[0]
            work_sheets.ExportAsFixedFormat(0,ctg_path_finance / Path(f"excel_{idx}.pdf"))
            sheets.Close()
            excel.Quit()             
